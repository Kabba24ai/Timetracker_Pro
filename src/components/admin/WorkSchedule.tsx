  ...
  const WorkSchedule: React.FC = () => {
        ...
        for (const employeeId of selectedEmployees) {
          ...
          for (let i = 0; i < totalDays; i++) {
            ...
            const dayShift = storeDay
              ? {
                start: storeDay.open ? storeDay.open.substring(0, 5) : '07:00',
      ...
      try {
        ...
        selectedEmployees.forEach(employeeId => {
          ...
          employeeWorkDays.forEach(day => {
            schedules.push({
              ...day,
              start_time: formatTimeTo24Safe(day.start_time) || '00:00',
              end_time: formatTimeTo24Safe(day.end_time) || '00:00',
            });
          });

        });

        const res = await api.post('/work-schedule/work-schedule/save', {
          schedules
        });

        if (!res.success) {
          toast.error(res.message || 'Failed to save schedule', { id: toastId });
          return;
        }

        toast.success('Work schedule saved successfully', { id: toastId });

      } catch (error) {

        console.error(error);
        toast.error('Error saving schedule', { id: toastId });
    ...
    const saveEdit = async () => {
      ...
      const key = `${editingCell.employeeId}-${editingCell.date}`;
      setUpdatingCell(key);

      const start = formatTimeTo24Safe(editValues.start_time || '');
      const end = formatTimeTo24Safe(editValues.end_time || '');
      ...
      try {
        const res = await api.post('/work-schedule/work-schedule/update', updatedDay);
    ...
    const toggleScheduled = async (employeeId: string, date: string) => {
      ...
      const day = workDays[employeeId].find(d => d.date === date);
      if (!day) {
        setUpdatingCell(null);
        return;
      }

      const nextScheduled = !day.is_scheduled;

      const updated = {
        employee_id: Number(employeeId),
        date: day.date,
        store_id: day.store_id ?? null,
        is_scheduled: nextScheduled,

        start_time: nextScheduled
          ? formatTimeTo24Safe(day.start_time)
          : null,

        end_time: nextScheduled
          ? formatTimeTo24Safe(day.end_time)
          : null,

        hours: nextScheduled
          ? calculateHours(day.start_time, day.end_time)
          : 0,
      ...
      try {
        const res = await api.post('/work-schedule/work-schedule/update', updated);
                                          ...
                                          <>
                                            ...
                                            <div className="flex flex-col items-center gap-1">
                                              <input
                                                ...
                                                className="w-22 px-1  border border-gray-300 rounded text-xs"
                                              />
                                              <input
                                                type="time"
